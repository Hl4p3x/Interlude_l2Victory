...
public class GameGuard {
    private static final Logger LOGGER = LoggerFactory.getLogger(GameGuard.class);
    private static final GameGuard _instance = new GameGuard();
    // CatsGuard
    private static final String catsguard = "config/catsguard.properties";
    private static int SERVER_KEY = 7958915;
    private static boolean ENABLED = true;
    private static String _hwid;
    private static boolean HWID;
    private Set<Hwid> _lockhwid;
    ...
    private GameGuard() {
        try {
            ...
            if (!ENABLED) {
                LOGGER.info("CatsGuard: disabled");
                return;
            }
            ...
            LOGGER.info("CatsGuard: Loaded " + _bannedhwid.size() + " banned hwid(s)");
        } catch (Exception e) {
            LOGGER.warn("CatsGuard: Error while loading config/catsguard.properties", e);
            ENABLED = false;
        }
    }
    ...
    public void initSession(GameClient cl, int[] data) {
        ...
        if (data[0] != SERVER_KEY) {
            if (LOG_OPTION.contains("NOPROTECT")) {
                LOGGER.info("CatsGuard: Client " + cl + " try to log with no CatsGuard");
            }
            cl.closeNow(false);
            return;
        }
        ...
        if (cl._reader == null) {
            if (LOG_OPTION.contains("HACK")) {
                LOGGER.info("CatsGuard: Client " + cl + " has no pre-authed state");
            }
            cl.closeNow(false);
            return;
        }
        if (_bannedhwid.contains(hwid)) {
            ((GuardReader) cl._reader)._checkChar = true;
        }
        ...
        if (max > 0 && ++nwindow > max) {
            if (LOG_OPTION.contains("SESSIONS")) {
                LOGGER.info("CatsGuard: To many sessions from hwid " + hwid);
            }
            cl.closeNow(false);
            return;
        }
        ...
        cl.setHwid(hwid);
        ((GuardReader) cl._reader).setKey(data);
    }
    ...
    public void Hwid() {
        ...
        try {
            ...
            LOGGER.error("Error while saving guard/Hwid.txt!", e);
        }
    }
    ...
    public void loadHwid() {
        ...
        try {
            ...
            LOGGER.error("Error while loading guard/BanHwid.txt!");
        }
    }
    ...
    private void saveHwidBan() {
        try {
            ...
            LOGGER.error("Error while saving guard/BanHwid.txt!", e);
        }
    }
    ...
    private class GuardReader implements GameClient.IExReader {
        private RC4 _crypt;
        ...
        private void Action(GameClient cl, String reason) {
            ...
            LOGGER.info("CatsGuard: Client " + cl + " use illegal software and will " + ON_HACK_ATTEMP + "ed. Reason: " + reason);
            cl.closeNow(false);
        }
        ...
        private void setKey(int[] data) {
            String key = "";
            for (int i = 0; i < 10; i++) {
                key += String.format("%X%X", data[1], GameGuard.SERVER_KEY);
            }
